...
$("body").click(function() {
	if (event.target.id === "deletor") {
		...
		ref = new Firebase("https://blistering-heat-398.firebaseio.com/songs.json/");
		ref.child(key).remove();
	}
});

// Filter by artist dropdown. Will show selected artist full song description(title, artist, album) in DOM
$("#selectArtist").on("click", "li", function (event) {
	var artistVal = $(event.target).text();
    console.log("artistVal", artistVal);
	$(".songp").show();
	console.log("yep");
	$(event.target).parents().siblings("#selectArtist").html(artistVal + '<span id="deletor" class="caret"></span>');
...
$("#album").on("click", "li", function (event) {
	...
	$(".songp").show();
	console.log("yep");
	$(event.target).parents().siblings("#selectArtist").html(albumVal + '<span id="deletor" class="caret"></span>');
	console.log("yep2", $(event.target).parents().siblings());
...
$("#button").click(function(){
	console.log("click");
	$("#selectArtist").siblings("#selectArtist").html("Artists" + '<span id="deletor" class="caret"></span>');
	$("#album").siblings("#selectArtist").html("Albums" + '<span id="deletor" class="caret"></span>');
	$(".songp").show();
});
...
// Returning to DOM songs to main content/artist to artist dropdown/album to album dropdown
	return {
		 songsIWantToAdd: function(songList) {
			...
			$("#selectArtist").html(artistTemplate(songList));
			console.log("selectArtist", artistTemplate(songList));
			// Appending album name to album dropdown
			$("#selectAlbum").html(albumTemplate(songList));
			console.log("selectAlbum", albumTemplate(songList));
			var seen = {};
			// Checking to see if current artist is duplicated in artist dropdown, if so, will be removed
			$('#selectArtist li').each(function() {
